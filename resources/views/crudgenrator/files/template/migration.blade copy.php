...
{ 
    ...
    public function up()
    {
        Schema::create('{{ $data["name"] }}', function (Blueprint $table) {
            ...
            $table->{{ $data['fields']['type'][$index] }}('{{ $item }}',[{!! $data['fields']['comment'][$index]  !!}])->comment('{!!  $data['fields']['comment'][$index]  !!}')->nullable();        @endif    @else        @if($required && $default && $data['fields']['input'][$index] != 'hidden')            @if($default == "now()")
            ...
            $table->{{ $data['fields']['type'][$index] }}('{{ $item }}')->comment('{!! $data['fields']['comment'][$index]  !!}')->nullable();        @endif @endif @if($cascade)
                
            $table->foreign('{{ $item }}')->references('{{ $data['fields']['ref_table'][$index] }}')->on('{{ $data['fields']['ref_col'][$index] }}')@if(array_key_exists('ref_on_update_'.$index,$data['fields']) && array_key_exists('ref_on_delete_'.$index,$data['fields']))->onUpdate('cascade')->onDelete('cascade')@elseif(array_key_exists('ref_on_update_'.$index,$data['fields']))
                ->onUpdate('cascade')@elseif(array_key_exists('ref_on_delete_'.$index,$data['fields']))->onDelete('cascade')@endif;  @endif @endforeach
              
            $table->timestamps(); @if(isset($data['softdelete']))
             
            $table->softDeletes(); @endif
