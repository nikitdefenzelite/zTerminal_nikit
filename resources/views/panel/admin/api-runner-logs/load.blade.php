<div class="card-body">
                ...
                <label for="">Show
                    <select name="length" class="length-input" id="length">
                        @foreach (tableLimits() as $limit)
                            <option
                                value="{{ $limit }}"{{ $apiRunnerLogs->perPage() == @$limit ? 'selected' : '' }}>
                                {{ $limit }}</option>
                        @endforeach
                    </select>
                    entries
    ...
    <div class="table-responsive">
        @include('panel.admin.api-runner-logs.table')
    </div>
</div>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="pagination mobile-justify-center">
                {{ $apiRunnerLogs->appends(request()->except('page'))->links() }}
            </div>
        </div>
        <div class="col-lg-4 mobile-mt-20">
            @if ($apiRunnerLogs->lastPage() > 1)
                <label class="d-flex justify-content-end mobile-justify-center" for="">
                    ...
                    <input type="number" class="w-25 form-control" id="jumpTo" name="page"
                        value="{{ $apiRunnerLogs->currentPage() ?? '' }}">
                </label>
            @endif
        </div>
    </div>
