<div class="table-controller mb-2">
    <div class="d-flex justify-content-between">
            ...
            <label for="">{{ __('show') }}
                <select name="length" class="length-input" id="length">
                    @foreach (tableLimits() as $limit)
                        <option value="{{ @$limit }}"{{ @$users->perPage() == @$limit ? 'selected' : '' }}>
                            {{ @$limit }}</option>
                    @endforeach
                </select>
                {{ __('Entry') }}
        ...
        <div>
            ...
            {{-- <a href="javascript:void(0);" id="print" data-url="{{ route('panel.admin.users.print') }}" data-rows="{{ json_encode($users) }}"class="btn btn-light btn-sm">Print</a> --}}
        </div>
    </div>
    <div class="d-flex justify-content-between">
        <div>
        ...
        <button type="button" class="off-canvas btn btn-light text-muted btn-icon"><i class="ik ik-filter ik-lg"></i>
        </button>
    </div>
</div>
